...
# intialise the temperature and epsilion
T = 100000
epsilion = 0.000001
...
# current is the randomly generated tour,
current = (random.sample(range(0, num_cities), num_cities))

best_tour = []

# objective function, working out the tour length
def objective(set):
    set_tour_length = 0
...
# enhancement inside of this function
def gen_enhanced(current_state):
    # lists to add successors and their weightings too
    successors  = []
    list_of_f_values = []
    # generates all successors of the state
    for i in range(0, num_cities):
        for j in range(0, num_cities):
            ...
            # don't want the current tour to be added, and make sure that the tour is not already in the tour
            if temp not in successors and temp != current_state:
                # add all the successors to the list
                successors.append(temp)
                list_of_f_values.append(objective(temp))
    
    # enhancement rather than choosing a completely random one, we randomly pick one of the best 5 tours
    copied_f = list_of_f_values.copy()
    # sorts the list an retrieves the smallest 5
    copied_f = sorted(copied_f)[:5]
    # makes a random choice of one of these 5 values
    choice = random.choice((copied_f))
    # then makes this choice equal to the tour generated
    value = successors[((list_of_f_values.index(choice)))]
    return value

tour = []
t = 0
while t < max_it and time.time() - start_time < 40:
    # T is the temperature, worked out by the previous temperature divided by beta (decay rate) 
    T = T * 0.5
    
    # if the temperature becomes too small then we return the best tour
    if T <= epsilion:
        # if temp is smaller than espilion we add the current city to the solution
        # as this means that the solution we have should be the best
        tour = current
        break
    
    else:
        # enhancement, a picks a random choice of the best 5 successors
        succ_tour = gen_enhanced(current)

        # objective function of succc - objective function of current
        # uses the value of the current state and the next state to work out the cost to get to this state
        change_E = -(objective(succ_tour) - objective(current))
        
        if change_E >= 0:
            # new current state is next_state change this to the new solution
            current = succ_tour

        else:
            # works out the probability of choosing if it is a worse tour
            probability = ((math.e) ** ((change_E) /T))

            # if the probability is under a certain criteria, we accept and see??
            if probability > random.uniform(0,1):
                current = succ_tour
    t = t + 1
    # finds the time for each iteration
    time_after_iteration = time.time() - start_time
    # breaks if an iteration takes more than 27.5 seconds
    if (time_after_iteration*2) > 55:
        tour = current
        print("broke")
        break

# once the number of iterations have been reached, the tour will be the best-tour so far,
# and tour_length is the length of it
tour = current
tour_length = objective(tour)
