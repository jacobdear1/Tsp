...
def gen_random(current_state):
        # lists to add successors and their weightings too
        successors  = []
        list_of_f_values = []
        # generates all successors of the initial state looks through the cities and makes swaps
        for i in range(0, num_cities):
            for j in range(0, num_cities):
        ...
        solution = random.choice((list_of_f_values))
        value = successors[((list_of_f_values.index(solution)))]
        return value
...
#for t in range(1,max_it):
    ...
    if T <= epsilion:
        ...
        # generate all successors of the current tour, and randomly select one
        succ_tour = gen_random(current)
        ...
        if change_E >= 0:
            ...
            # calculate the probability, will always be worse 
            probability = ((math.e) ** ((change_E) /T))
            
            # randomly choose the probability that it will be accepeted
