...
# uses nearest_neighbour (basic greedy to work out the inital pheromone levels)
...
# enhancement performs a hill climbing search on the tour supplied
def hill_climbing(current_state):
    # lets it perform hill climbing for 10 seconds?
    hc_end_time = start_time + 5
    x = 0
    # this affects the running time
    max_iterations = 1
    # repeat until we reach a certain number of iterations
    # or the time is up, if the time is greater than 20 we will just stop
    while x < max_iterations and time.time() < hc_end_time:

        successors  = []
        list_of_f_values = []
        # generates all successors of the initial state
        #print("current", current_state)
        temp = current_state

        # looks through the cities and makes swaps
        for i in range(0, num_cities):
            for j in range(0, num_cities):
                temp = current_state.copy()
                # make sure we don't swap the same pair twice
                if i < j:
                    # swap the pair, set the temp variable equal as well, as this will change continously
                    temp[i], temp[j] = current_state [j], current_state [i]
                # don't want the current tour to be added, 
                # and make sure that the tour is not already in the tour
                if temp not in successors and temp != current_state:
                    # add all the successors to the list
                    successors.append(temp)
                    # add the path_cost into a list, so the path_cost has the same index postion as the tour if we need to reference it   
                    list_of_f_values.append(calculate_path_length(temp))

        # select the successor with the smallest f value (path_cost)
        # look through the list_of_f_values, find the smallest one
        min_cost = min(list_of_f_values)

        min_cost_index = list_of_f_values.index(min_cost)

        # need to look through the successors and make a decison 

        # objective function f is the path_cost 
        # if this sucessor has a worse f value (bigger path cost than the current best) then we keep current 

        if min_cost > calculate_path_length(current_state):
            current_state = current_state
        # else if this successor has a better f value (smaller path cost) then we replace the current_state with the successor
        else:
            current_state = successors[min_cost_index]

        # go again for the next iteration
        x = x + 1
...
def objective(set):
    set_tour_length = 0
    # works out the tour length for the city sets
    for i in range(0, num_cities - 1):
        set_tour_length = set_tour_length + dist_matrix[set[i]][set[i + 1]]
    set_tour_length = set_tour_length + dist_matrix[set[num_cities - 1]][set[0]]
    
    # returns the 'cost', tour_length, which we can compare
    return set_tour_length
...
# gets the returned values of the nearest neighbour algorithim
tour, tour_length = nearest_neighbour(dist_matrix)

# pheromone
# works out the length of tour produced by nearest neighbour algorithm
Lnn = tour_length
intial_pheromone = 1/Lnn
...
# intialise pheromone level on each edge to be intial pheromone
...
# creates a random path, of unique values
best_tour = (random.sample(range(0, num_cities), num_cities))

# works out the tour_length of our randomly chosen 'best' path
best_tour_length = calculate_path_length(best_tour)
...
while t < max_it and time.time() - start_time < 55: # repeat for max_it iterations
    ...
    for ant in range(1,num_of_ants): # for each ant, in the range of number of total ants
        ...
        if time.time() - start_time > 40:
            ...
            # ant is the first city, or the next_city to be looked at once the next_city has been found
                ...
                if len(solution) != num_cities:
                    ...
                    for j in range(num_cities):
                        ...
                        if j not in Fk:
                            ...
                            # as specified in the lectures, if the distance of an edge is 0, add some small amount to represent it, 
                            if dist_matrix[ant][j] == 0:
                                hueristic_desirability = 0.01

                            else:
                                # works out the heurisitc desirability by dividing by the length of the edge
                    ...
                    for value in list_of_probs:
                        # divides the value by the sum of the probailites of all cities, 
                        # as this is how we work out probability, as mentioned in lectures
                        value = value / sum
                        # adds to a list so we can compare these and extract the one of largest probability
    ...
    for path in k_paths:
        ...
        # adds to a list, so we can compare and find the minimum
        ...
        # local search stage to try and improve the tours before we've updated the pheromone perform on the best tour 
        # found so far, reduces time complexity
        updated_tour = hill_climbing(min_path)
        # if the best tour found by hill climbing is shorter then we use this as our new tour
        if calculate_path_length(updated_tour) < calculate_path_length(min_path):
            # then we update the path in the list of all solutions
            min_path = updated_tour
    ...
    # update the values of delta_tau for those cities in any of the N tours generated, amount of pheromone is 
        for city in range(0,num_cities-1):
            ...
            pheromone_value = 1 / k_paths_lengths
    ...
    # deposit/evaporate pheromone on edges -> after best solution is found
    ...
    tour = best_tour
    tour_length = calculate_path_length(tour)
